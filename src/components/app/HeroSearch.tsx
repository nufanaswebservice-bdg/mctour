    ...
    <>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-b from-blue-50/80 via-white/60 to-white/90" />
        </div>
          ...
          >
                    ...
                    <div className="rounded-2xl bg-background/60 border border-primary/5 p-3">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <input type="date" value={checkIn} onChange={(e) => setCheckIn(e.target.value)} className="w-full px-3 py-2.5 rounded-xl bg-white border border-primary/10 text-xs text-dark-text outline-none focus:border-primary/30" />
                        </div>
                        <div>
                          <label className="text-[10px] text-muted block mb-1">Check-out</label>
                          <input type="date" value={checkOut} onChange={(e) => setCheckOut(e.target.value)} min={checkIn} className="w-full px-3 py-2.5 rounded-xl bg-white border border-primary/10 text-xs text-dark-text outline-none focus:border-primary/30" />
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="space-y-3">
                        {[
                          ...
                          <div key={item.field} className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-3">
                              <button onClick={() => updateGuest(item.field, -1)} disabled={guests[item.field] <= item.min} className="w-8 h-8 rounded-full border border-primary/20 flex items-center justify-center active:scale-90 disabled:opacity-30"><Minus size={14} className="text-primary" /></button>
                              <span className="text-sm font-bold text-dark-text w-5 text-center">{guests[item.field]}</span>
                              <button onClick={() => updateGuest(item.field, 1)} className="w-8 h-8 rounded-full border border-primary/20 flex items-center justify-center active:scale-90"><Plus size={14} className="text-primary" /></button>
                            </div>
                          </div>
                        ))}
                      </div>
            ...
            <motion.button onClick={handleSearch} whileTap={{ scale: 0.97 }} className="relative w-full h-12 rounded-2xl overflow-hidden shadow-lg shadow-primary/25">
              ...
              <span className="relative z-10 flex items-center justify-center gap-2 text-white font-bold text-sm">
                <Search size={16} /> Cari Paket Tour
              </span>
            </motion.button>
          </motion.div>
        </div>
      ...
      {/* Running Text Promo */}
      <Link href="/event-organizer" className="block">
        <div className="bg-[#0A2E7A] py-2.5 overflow-hidden cursor-pointer group">
          <div className="animate-marquee group-hover:[animation-play-state:paused] whitespace-nowrap flex">
            <span className="text-white text-xs font-semibold mx-8">
              🔥 PROMO DISCOUNT 10% GATHERING BULAN AGUSTUS - SEPTEMBER 2026 UNTUK CORPORATE, KLIK DISINI
            </span>
            <span className="text-white text-xs font-semibold mx-8">
              🔥 PROMO DISCOUNT 10% GATHERING BULAN AGUSTUS - SEPTEMBER 2026 UNTUK CORPORATE, KLIK DISINI
            </span>
